...
// import { trpc } from "@/lib/trpc-client"; // Will be used when API is ready
import { useOrganizationStore } from "@/lib/stores/organization";
import { useRouter } from "next/navigation";

export type OrganizationChatProfileProps =
  React.HTMLAttributes<HTMLDivElement> & {
    paginate?: (direction: number) => void;
  };

const OrganizationChatProfile = React.forwardRef<
  HTMLDivElement,
  OrganizationChatProfileProps
>(({ paginate, ...props }, ref) => {
  ...
  // In a real app, this would fetch conversation data
  // For now, using placeholder data that matches the conversationId
  const displayUser = conversationId
    ? {
        id: "user1",
        name: "John Doe",
        username: "johndoe",
  ...
  // In a real app, this would fetch messages with media
  ...
  React.useEffect(() => {
    if (conversationId && organizationId) {
      const savedNotes = localStorage.getItem(
        `org-chat-notes-${organizationId}-${conversationId}`,
      );
      setNotes(savedNotes || "");
    }
                ...
                <div className="flex flex-col items-center px-4 pt-2 text-center">
                  ...
                  <div className="mt-2 py-5 flex gap-2">
                    <Button
                      ...
                      color="primary"
                      onPress={() =>
                        router.push(`/users/${displayUser.username}`)
                      }
...
OrganizationChatProfile.displayName = "OrganizationChatProfile";

export default OrganizationChatProfile;
