...
export type AdminSidebarProps = Omit<
	ListboxProps<AdminSidebarItem>,
	"children" | "onSelect"
> & {
	items: AdminSidebarItem[];
	isCompact?: boolean;
...
const AdminSidebar = React.forwardRef<HTMLElement, AdminSidebarProps>(
	(
		{
			items,
			isCompact,
		...
		const renderNestItem = React.useCallback(
			(item: AdminSidebarItem) => {
				const isNestType =
					item.items &&
					item.items?.length > 0 &&
					item?.type === AdminSidebarItemType.Nest;
					...
					<ListboxItem
						...
						startContent={
							isCompact || isNestType ? null : item.icon ? (
								<Icon
									className={cn(
										"text-default-500 group-data-[selected=true]:text-primary-foreground group-hover:text-background",
										iconClassName,
									)}
									icon={item.icon}
									width={24}
		...
		return (
			<Listbox
				...
				classNames={{
					...classNames,
					list: cn("items-center gap-2", classNames?.list),
				}}
				itemClasses={{
					...itemClasses,
					base: cn(
						"px-3 min-h-11 rounded-large h-[44px] hover:text-background data-[selected=true]:bg-primary mb-1",
						itemClasses?.base,
					),
					title: cn(
					...
					wrapper: cn(
						"group-hover:text-background group-data-[selected=true]:text-primary-foreground",
					),
				}}
				items={items}
