    ...
    <div style={{ fontFamily: "'Oswald', sans-serif", background: "#020606", minHeight: "100dvh", position: "relative", overflow: "hidden", display: "flex", alignItems: "stretch", flexDirection: "row" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes floatSU { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-8px); } }
      ...
      <div className="su-left" style={{ flex: "0 0 42%", position: "relative", zIndex: 3, display: "flex", flexDirection: "column", justifyContent: "space-between", padding: "clamp(32px, 5vw, 64px)" }}>
        ...
        <div>
          ...
          <div className="su-a4">
            {PERKS.map((p, i) => (
              <div key={i} className="su-perk" style={{ animationDelay: `${0.4 + i * 0.1}s` }}>
      ...
      <div className="su-glass" style={{ flex: 1, position: "relative", zIndex: 3, display: "flex", alignItems: "center", justifyContent: "center", padding: "clamp(24px, 5vw, 56px)", overflowY: "auto" }}>
        <div className="su-safe" style={{ width: "100%", maxWidth: 420 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {/* Name */}
            <div className="su-a5">
              <label className={`su-label${focus === "name" ? " act" : ""}`}>Full Name</label>
              <div className={`su-input-wrap${focus === "name" ? " focused" : ""}`}>
                <input type="text" placeholder="Your full name" value={form.name} onChange={set("name")} onFocus={() => setFocus("name")} onBlur={() => setFocus(null)} autoComplete="name" />
              </div>
            </div>

            {/* Username */}
            <div className="su-a5">
              <label className={`su-label${focus === "username" ? " act" : ""}`}>Username</label>
              <div className={`su-input-wrap${focus === "username" ? " focused" : ""}`}>
                <input type="text" placeholder="Choose a username" value={form.username} onChange={set("username")} onFocus={() => setFocus("username")} onBlur={() => setFocus(null)} autoComplete="username" />
              </div>
            </div>

            {/* Email */}
            <div className="su-a5">
              <label className={`su-label${focus === "email" ? " act" : ""}`}>Email Address</label>
              <div className={`su-input-wrap${focus === "email" ? " focused" : ""}`}>
                <input type="email" placeholder="you@example.com" value={form.email} onChange={set("email")} onFocus={() => setFocus("email")} onBlur={() => setFocus(null)} autoComplete="email" />
              </div>
            </div>
